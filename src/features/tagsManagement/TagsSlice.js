...
export const createNewTag = createAsyncThunk('tagsManagement/createNewTag', async (req) => {
...
export const findTagByTitle = createAsyncThunk('tagsManagement/findTagByTitle', async (tagTitle) => {
    const res = await axios.get(`https://hoanghy.tech/api/v1/tag/${tagTitle}`)
    return res
})

export const findAllTagsOfUser = createAsyncThunk('tagsManagement/findAllTagsOfUser', async () => {
    const accessToken = await auth.currentUser.getIdToken();
    const res = await axios.post('https://hoanghy.tech/api/v1/tag',
    { headers: { Authorization: `Bearer ${accessToken}`}})
    return res
